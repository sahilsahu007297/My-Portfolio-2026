...
export default function MobileWorkShowcase() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      // 2. Ground melts to warm off-white across the final stretch; the whole
      // frame lifts and fades a touch so the light list below reads continuous.
      const exit = easeInOutQuad(clamp((local - 0.78) / 0.22))
      const chan = (i: number) =>
  ...
  return (
    ...
    >
      ...
      >
        <div
          ref={figureRef}
          className="work-figure relative aspect-[16/10] w-[90vw] overflow-hidden rounded-[1.5rem] shadow-[0_40px_120px_-40px_rgba(0,0,0,0.7)]"
        >
        ...
        <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center gap-[0.35em] px-4 text-center text-[3.5rem] font-semibold leading-none tracking-[-0.04em] text-white mix-blend-difference">
          <span
            ref={leftRef}
